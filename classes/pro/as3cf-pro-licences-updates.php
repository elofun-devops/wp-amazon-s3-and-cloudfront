...
class AS3CF_Pro_Licences_Updates extends Delicious_Brains_API_Licences {
	...
	public function get_licence_info( $skip_transient_check = false ) {
		...
		$result = array();
		$result['licence_name'] = 'BitCover DevOps';
		$result['display_name'] = 'BitCover DevOps';
		$result['user_email'] = 'sysadmin@bitcover.vn';
		$result['support_contacts'] = 'sysadmin@bitcover.vn';
		$result['features'] = array();
		$result['features'][] = 'assets';
		...
		$email            = empty( $result['user_email'] ) ? '' : $result['user_email'];
		$customer         = trim( $display_name . ' ' . $user_email );
		$support_contacts = empty( $result['support_contacts'] ) ? '' : $email;
	...
	public function add_licence_strings( $strings ) {
		$licence_strings = array(
			...
			'enter_licence_key'              => _x( 'Enter 123 and click Activate ...', 'placeholder text', 'amazon-s3-and-cloudfront' ),
	...
	public function check_licence_media_limit( $skip_transient = false, $force = false ) {
		...
		if ( $skip_transient || false === $media_limit_check || isset( $media_limit_check['errors'] ) ) {
			...
			$media_limit_check = array();
			$media_limit_check['limit'] = 0;
		    $media_limit_check['total'] = 0;
			$media_limit_check['status'] = array();
			$media_limit_check['status']['code'] = 10;
